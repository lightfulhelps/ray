...
stories.add('Default', () => {
  ...
  return <Container>
      ...
      tags
    }} actions={[{ children: 'Approve', onClick: action('Approve') }, { children: 'Preview', onClick: action('Preview') }, { children: 'Edit', onClick: action('Edit') }, { children: 'Delete', onClick: action('Delete') } // { children: 'Reschedule', onClick: action('Reschedule') },
    ]} isDraft={boolean('Draft', false)} isInvalid={boolean('Invalid', false)} isImported={boolean('Imported', false)} metaPreview={showMeta && {
      title: text('Meta Title', "National Living Wage 'fails to cover families' basic needs"),
      description: text('Meta Description', "Low-paid parents working full-time do not earn enough to meet their families' needs, a charity says."),
      image: text('Meta Image', 'https://ichef.bbci.co.uk/news/1024/branded_news/81C6/production/_98022233_gettyimages-842414672-1.jpg'),
      url: text('Meta URL', 'https://www.bbc.co.uk/news/business-45242008')
    }} notesAction={action('Notes')} notesCount={3} errors={boolean('Show errors', false) && ['Error string', 'Error string two with a much longer string than the other one', 'Error string two with a much longer string than the other one kinda long', 'Error string two with a much longer string than the other one much much longer its long']} />
    </Container>;
});