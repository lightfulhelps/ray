...
stories.add('Input', () => <Container>
    <h1 className="my-4">Search Input</h1>
    <Row>
      <Col className="mb-2">
        <SearchInput value={text('Search', '')} onChange={action('Change')} onSearch={action('Search')} placeholder={text('Placeholder', 'Search...')} />
      </Col>
    </Row>
  </Container>);

stories.add('Suggest', () => <Container>
    <h1 className="my-4">Search Suggest</h1>
    <Row>
      <Col className="mb-2">
        <SearchSuggest className="w-100" isLoading={boolean('Loading', false)} isOpen limit={number('Limit', 5)} onClear={action('Clear')} onRemove={action('Remove')} onSelect={action('Select')} options={boolean('Has options?', true) ? breeds : []} search={text('Search', '')} title={text('Title', 'Recent Searches')} />
      </Col>
    </Row>
  </Container>);